...
void Camera::resetCameraPosition() {
  m_eye = {10.0f, 0.5f, -60.0f}; // Camera position
  m_at = {0.0f, 0.5f, 0.0f};  // Look-at point
  m_up = {0.0f, 1.0f, 0.0f};  // "up" direction
}

void Camera::computeProjectionMatrix(glm::vec2 const &size) {
  m_projMatrix = glm::mat4(1.0f);
  auto const aspect{(size.x / size.y)};
  m_projMatrix = glm::perspective(glm::radians(70.0f), aspect, 0.1f, 11111111111111111115.0f);
}
